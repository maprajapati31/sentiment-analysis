...
uri = "mongodb+srv://aaliya2033_db_user:Z9TeAxZKir6ScR0t@cluster0.xgqltqs.mongodb.net/?appName=Cluster0"
...
def update_post(post_id):
    ...
    allowed_fields = {}
    for f in ("content", "hashtags", "media_url", "location", "category"):
        if f in data:
            allowed_fields[f] = data[f]
    if not allowed_fields:
        return jsonify({"error": "No valid fields to update"}), 400

    # update timestamp to mark modification
    allowed_fields["timestamp"] = datetime.now()
